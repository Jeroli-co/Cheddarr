...
import { IMediaServerSeries } from "../../../modules/media-servers/models/IMediaServerMedia";
import { PlexService } from "../../../modules/media-servers/plex/services/PlexService";

type SeriesCardProps = {
  series: IMediaServerSeries | null;
};
...
const SeriesCard = ({ series }: SeriesCardProps) => {
  ...
  const [seriesInfo, setSeriesInfo] = useState<IMediaServerSeries | null>(null);

  useEffect(() => {
    if (!series) {
      PlexService.GetSeries(parseInt(id, 10)).then((res) => {
        if (res.error === null) setSeriesInfo(res.data);
      });
    } else {
  ...
  if (!seriesInfo || seriesInfo.id !== parseInt(id, 10))
        ...
        <RowLayout alignItems="flex-start">
          ...
          <Container paddingRight="1%" paddingLeft="1%">
            ...
            >
              ...
              <PlexButton
                onClick={() => window.open(seriesInfo.webUrl[0])}
                text="Open in plex"
              />
            </RowLayout>
            {seriesInfo.summary && (
