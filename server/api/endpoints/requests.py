...
def add_movie_request(request: dict):
    ...
    existing_movie = MovieRequest.find(
    ...
    movie_request = MovieRequest.create(
...
@login_required
@body(SeriesChildRequestCreateSchema)
@jsonify_with(SeriesChildRequestSchema)
def add_series_request(request):
    ...
    series_request = SeriesRequest.find(tvdb_id=tvdb_id, requested_user=requested_user)
    if series_request is None:
        series_request = SeriesRequest.create(
            tvdb_id=tvdb_id, requested_user=requested_user, series_type=series_type
        )

    existing_children = SeriesChildRequest.find_all(series_id=series_request.id)
    ...
    child_request = SeriesChildRequest.create(
...
def update_series_request(args: dict, id: int):
    request = SeriesChildRequest.find(id=id)
    if request is None:
        raise NotFound("This request does not exist.")
    if request.parent.requested_user != current_user:
...
def delete_series_request(id: int):
    ...
    if (
        request.requesting_user != current_user
        or parent_request.requested_user != current_user
    ):
        raise Forbidden("This is not one of your requests.")
    request.delete()
    if not parent_request.children:
