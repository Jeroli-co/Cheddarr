...
from server.services.plex import PlexService
...
async def get_plex_account_servers(
    ...
    plex_service: PlexService = Depends(deps.get_service(PlexService)),
) -> list[PlexServer]:
    if current_user.plex_api_key is None:
        raise HTTPException(status.HTTP_404_NOT_FOUND, "No Plex account linked to the user.")

    return await plex_service.get_account_servers(current_user.plex_api_key)
...
async def get_plex_settings(
    ...
    return await plex_service.get_settings()
...
async def add_plex_setting(
    ...
    setting = await plex_service.get_setting(server_id=setting_in.server_id)
...
async def update_plex_setting(
    ...
    setting = await plex_setting_repo.find_by(id=setting_id).one()
    if setting is None:
    ...
    return await plex_setting_repo.update(setting, setting_in)
...
async def get_plex_libraries(
    ...
    libraries = await plex_service.get_server_library_sections(setting.host, setting.port, setting.ssl, setting.api_key)
    if libraries is None:
        raise HTTPException(status.HTTP_503_SERVICE_UNAVAILABLE, "Failed to get Plex libraries.")

    for library in libraries:
        library.enabled = any(int(lib.library_id) == library.library_id for lib in setting.libraries)
...
async def update_plex_setting_libraries(
    ...
    for library_in in libraries_in:
        setting_library = next((lib for lib in setting.libraries if int(lib.library_id) == library_in.library_id), None)
...
async def get_radarr_setting_instance_info(
    setting_id: str,
    radarr_setting_repo: RadarrSettingRepository = Depends(deps.get_repository(RadarrSettingRepository)),
) -> RadarrInstanceInfo:
    setting = await radarr_setting_repo.find_by(id=setting_id).one()
    if setting is None:
...
async def get_radarr_settings(
    ...
    return await radarr_setting_repo.find_by().all()
...
async def update_radarr_setting(
    ...
    return await radarr_setting_repo.update(setting, setting_in)
...
async def get_sonarr_setting_instance_info(
    setting_id: str,
    sonarr_setting_repo: SonarrSettingRepository = Depends(deps.get_repository(SonarrSettingRepository)),
) -> SonarrInstanceInfo:
    setting = await sonarr_setting_repo.find_by(id=setting_id).one()
    if setting is None:
...
async def get_sonarr_settings(
    ...
    return await sonarr_setting_repo.find_by().all()
...
async def update_sonarr_setting(
    ...
    return await sonarr_setting_repo.update(setting, setting_in)
