...
class RequestService(BaseService):
    ...
    @classmethod
    def unify_series_request(cls, series_request: SeriesRequest, request_in: SeriesRequestCreate) -> None:
        ...
        for season_request_in in request_in.season_requests:
            ...
            episode_requests = []
            for episode_request in season_request_in.episode_requests:
                episode_requests.append(episode_request.to_orm(EpisodeRequest))
    ...
    @staticmethod
    async def send_series_request(request: SeriesRequest, provider: MediaProviderSetting) -> None:
        if provider.service_name == ExternalServiceName.sonarr:
            await sonarr.send_request(request)

    @staticmethod
    async def send_movie_request(request: MovieRequest, provider: MediaProviderSetting) -> None:
